...
import { Task, User, Project, TaskPriority, TaskStatus } from './types';
...
const App: React.FC = () => {
  ...
  // Fixed: toggleTaskSelection implementation added
  ...
  useEffect(() => {
    // Detect Public Share Route Simulation
  ...
  return (
    <WorkspaceLayout
      ...
      onOpenVoiceCommander={() => setIsVoiceCommanderOpen(true)}
      onOpenVisionModal={() => setIsVisionModalOpen(true)}
      onAddProject={() => setIsProjectModalOpen(true)}
    >
      <PresenceOverlay users={allUsers} />
      ...
      {currentView === 'board' ? (
        <KanbanView 
          ...
          // Fixed: Added onToggleTimer prop passing to KanbanView
          onToggleTimer={toggleTimer}
          handleStatusUpdate={(id, s) => updateStatus(id, s, user.displayName)}
          moveTask={moveTask}
      ...
      <SelectionActionBar 
        ...
        onBulkPriority={(p) => { bulkUpdateTasks(selectedTaskIds, { priority: p }); setSelectedTaskIds([]); }}
        onBulkStatus={(s) => { bulkUpdateTasks(selectedTaskIds, { status: s }); setSelectedTaskIds([]); }}
        onBulkAssignee={(u) => { bulkUpdateTasks(selectedTaskIds, { assigneeId: u }); setSelectedTaskIds([]); }}
      ...
      <GlobalModals 
        ...
        isVoiceCommanderOpen={isVoiceCommanderOpen}
        setIsVoiceCommanderOpen={setIsVoiceCommanderOpen}
        isVisionModalOpen={isVisionModalOpen}
        setIsVisionModalOpen={setIsVisionModalOpen}
        ...
        handleAddProject={(n, d, c, m) => {
            const proj = projectService.createProject(n, d, c, m);
            setProjects([...projects, proj]);
            setActiveProjectId(proj.id);
            setIsProjectModalOpen(false);
        }}
        handleUpdateTask={(id, u) => {
            updateTask(id, u, user.displayName);
            if(selectedTask?.id === id) setSelectedTask({...selectedTask, ...u});
        }}
        handleCommentOnTask={(id, t) => {
            const updated = addComment(id, t);
            if(updated) setSelectedTask(updated);
        }}
        deleteTask={deleteTask}
        applyAISuggestions={applyAISuggestions}
        handleGeneratedTasks={(g) => {
            g.forEach(x => createTask(x.title, x.description, TaskPriority.MEDIUM, ['Vision Scan'], undefined, activeProjectId || 'p1'));
        }}
