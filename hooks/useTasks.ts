...
export const useTasks = (user: User | null, activeProjectId?: string) => {
  ...
  useEffect(() => {
    const handleUndoRedo = (e: KeyboardEvent) => {
      if ((e.metaKey || e.ctrlKey) && e.key === 'z') {
        ...
        if (nextState) {
          setTasks(nextState);
          // Persist the state back to service
          taskService.reorderTasks(user?.id || '', nextState);
        }
      }
    };
  ...
  const createTask = (title: string, description: string, priority: TaskPriority, tags: string[] = [], dueDate?: number, projectId: string = 'p1', assigneeId?: string) => {
    ...
    const newTask = taskService.createTask(user.id, projectId, title, description, priority, tags, dueDate, assigneeId);
    refreshTasks();
  };
  ...
  const updateStatus = (id: string, status: TaskStatus, username?: string) => {
    ...
    const updated = taskService.updateTaskStatus(user.id, id, status, username);
    setTasks(updated);
  };
  ...
  const bulkUpdateTasks = (ids: string[], updates: Partial<Task>) => {
    ...
    let current = [...tasks];
    ids.forEach(id => {
      current = taskService.updateTask(user.id, id, updates, user.displayName);
    });
    setTasks(current);
  };
  ...
  const bulkDeleteTasks = (ids: string[]) => {
    ...
    let current = [...tasks];
    ids.forEach(id => {
      current = taskService.deleteTask(user.id, id);
    });
    setTasks(current);
  };
  ...
  const updateTask = (id: string, updates: Partial<Omit<Task, 'id' | 'userId' | 'createdAt' | 'order'>>, username?: string) => {
    if (!user) return;
    historyManager.push(tasks);
    const updated = taskService.updateTask(user.id, id, updates, username);
    setTasks(updated);
  };

  const addComment = (taskId: string, text: string) => {
    if (!user) return;
    const updated = taskService.addComment(user.id, taskId, text, user.displayName);
    setTasks(updated);
  ...
  const moveTask = (taskId: string, targetStatus: TaskStatus, targetTaskId?: string) => {
    ...
    setTasks(prevTasks => {
      ...
      taskService.reorderTasks(user.id, reordered);
  ...
  const deleteTask = (id: string) => {
    if (!user) return;
    historyManager.push(tasks);
    const updated = taskService.deleteTask(user.id, id);
    setTasks(updated);
  };

  // Fixed: Added toggleTimer implementation to useTasks hook
  const toggleTimer = (id: string) => {
    if (!user) return;
    const updated = taskService.toggleTimer(user.id, id);
    setTasks(updated);
  };
  ...
  const applyAISuggestions = () => {
    if (!user || !activeTaskId || !aiSuggestions) return;
    historyManager.push(tasks);
  ...
  return {
    ...
    bulkUpdateTasks,
    bulkDeleteTasks,
    toggleTimer // Added missing toggleTimer to the return object
  };
};
