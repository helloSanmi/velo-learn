...
const Header: React.FC<HeaderProps> = ({ user, onLogout, onNewTask, onReset, onRefreshData, onToggleSidebar, onOpenSettings, onOpenTaskFromNotification, onlineCount, isOnline }) => {
  ...
  return (
    <header className="flex-none w-full bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 md:px-8 py-2.5 sticky top-0 z-[55]">
      <div className="max-w-[1800px] mx-auto flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button 
            onClick={onToggleSidebar}
            className="p-2 -ml-1 text-slate-500 hover:bg-slate-100 rounded-lg lg:hidden transition-all active:scale-95"
          >
