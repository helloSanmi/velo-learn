...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, initialTab = 'general', user }) => {
  ...
  const renderContent = () => {
    switch (activeTab) {
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex flex-col items-center gap-6 p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100">
               ...
               <div className="text-center">
                 ...
                 <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{org?.name} • {user?.role === 'admin' ? 'Admin' : 'Member'}</p>
               </div>
            </div>
            <div className="space-y-4">
               <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest px-1">Account</h3>
      ...
      case 'general':
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="space-y-4">
               <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest">AI</h3>
               <div className="flex items-center justify-between p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 hover:bg-white hover:shadow-sm transition-all group">
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium">Suggest subtasks and identify risk.</p>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
               <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest">Sync</h3>
               <div className="flex items-center justify-between p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 hover:bg-white hover:shadow-sm transition-all group">
                  <div className="flex gap-4">
                    <div className="p-2.5 bg-white rounded-xl shadow-sm text-indigo-500 group-hover:scale-110 transition-transform">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium">Keep updates in sync across the workspace.</p>
                    </div>
                  </div>
      ...
      case 'notifications':
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest px-1">Notifications</h3>
            <div className="flex items-center justify-between p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 hover:bg-white hover:shadow-sm transition-all group">
              <div className="flex gap-4">
                <div className="p-2.5 bg-white rounded-xl shadow-sm text-indigo-500 group-hover:scale-110 transition-transform">
            ...
            <div className="p-8 bg-indigo-50/50 rounded-[2rem] border border-indigo-100 flex flex-col items-center text-center gap-3">
              ...
              <p className="text-sm font-black text-slate-900">Mobile Notifications</p>
      ...
      case 'security':
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest px-1">Security</h3>
            <div className="p-8 border-2 border-indigo-100 bg-indigo-50 rounded-[2.5rem] flex flex-col items-center text-center gap-4 relative overflow-hidden group">
              ...
              <div>
                <p className="text-lg font-black text-slate-900">Workspace Security</p>
                <p className="text-sm text-indigo-700/80 font-bold mt-1">Workspace ID: {org?.id.slice(0, 8)}...-SEC</p>
              </div>
              <p className="text-xs text-slate-600 font-medium leading-relaxed max-w-xs">Your organization data is isolated at the database level and protected with encryption and access controls.</p>
              <Button size="md" variant="secondary" className="mt-2 rounded-xl px-8 shadow-indigo-200">Enable Enhanced Security</Button>
            </div>
          </div>
        );
      case 'appearance':
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest">Appearance</h3>
            <div className="flex items-center justify-between p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100">
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">Reduce spacing to fit more content.</p>
                  </div>
                </div>
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium mt-1">Workspace ID: {org?.id.slice(0,8)}... • Created {new Date(org?.createdAt || 0).toLocaleDateString()}</p>
              </div>
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-0 md:p-4 bg-slate-900/45 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-4xl rounded-t-2xl md:rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-6 md:zoom-in-95 duration-200 max-h-[90vh] md:max-h-[80vh] flex flex-col md:flex-row border border-slate-200">
